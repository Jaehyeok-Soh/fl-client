#include "pch.h"
#include "Lianhuo_XSpace.h"
#include "PhysicsCollider.h"
#include "PhysicsRigidBody.h"
#include "GameInstance.h"

CLianhuo_XSpace::CLianhuo_XSpace(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: Super(pDevice, pDeviceContext)
{

}

CLianhuo_XSpace::CLianhuo_XSpace(const CLianhuo_XSpace& rhs)
	: Super(rhs)
{

}

HRESULT CLianhuo_XSpace::Initialize_Prototype()
{
	if (FAILED(Super::Initialize_Prototype()))
		return E_FAIL;

	return S_OK;
}

HRESULT CLianhuo_XSpace::Initialize(void* pArg)
{
	if (FAILED(Super::Initialize(pArg)))
		return E_FAIL;

	if (FAILED(Ready_Modules()))
		return E_FAIL;

	return S_OK;
}

HRESULT CLianhuo_XSpace::Ready_Modules()
{
	wstring wstrDefaultPrototypeTag = L"Prototype_GameObject_Effect";

	// Effect
	{
		// WARNING
		{
			if (FAILED(Add_EffectModule(
				0 /* static */,
				"WarningCircle1",
				L"Prototype_GameObject_Effect_WarningCircle",
				ENUM_TO_UINT(EState::WARNING))))
				return E_FAIL;
		}

		// STRIKE
		{
			if (FAILED(Add_EffectModule(
				0 /* static */,
				"Boss_LianHuo_XSpace",
				wstrDefaultPrototypeTag,
				ENUM_TO_UINT(EState::STRIKE))))
				return E_FAIL;
		}
	}
	// Collider
	{
		PHYSICSCOLLIDER_DESC colliderDesc{};
		colliderDesc.eShape = EPhysicsShape::BOX;
		colliderDesc.eFilterLayer = PHYSICSFILTERGROUP::MONSTER_SKILL_PROJECTTILE;
		//cloneDesc.bIsSkillTrigger = true;
		colliderDesc.iFilterMask =
		{
			PHYSICSFILTERGROUP::Enum::PLAYER
			| PHYSICSFILTERGROUP::Enum::MAP
		};
		colliderDesc.bIsTrigger = true;
		colliderDesc.bSetOnlyFilter = false;
		colliderDesc.bIsActive = true;
		colliderDesc.vCenter = { 0.f, 1.f, 0.f };
		colliderDesc.vExtents = { 4.f, 3.f, 10.f };
		colliderDesc.strAttackPresetTag = "Xibi_Thunder";
		PHYSICSMATERIAL_DESC mtrlDesc{};
		mtrlDesc.eMaterial = EPhysicsMaterial::CONCRETE;
		colliderDesc.tMaterial = mtrlDesc;

		PHYSICSRIGIDBODY_DESC rigidbodyDesc{};
		rigidbodyDesc.eType = EPhysicsActorType::KINEMATIC;
		rigidbodyDesc.detection = EPhysicsCollisionDetection::DISCRETE;
		rigidbodyDesc.bUseGravity = false;
		rigidbodyDesc.bIsKinematic = true;

		// STRIKE
		{
			if (FAILED(Add_CollideModule(
				ENUM_TO_UINT(EState::STRIKE),
				&colliderDesc,
				&rigidbodyDesc)))
				return E_FAIL;
		}
	}
	// Collider
	{
		PHYSICSCOLLIDER_DESC colliderDesc{};
		colliderDesc.eShape = EPhysicsShape::BOX;
		colliderDesc.eFilterLayer = PHYSICSFILTERGROUP::MONSTER_SKILL_PROJECTTILE;
		//cloneDesc.bIsSkillTrigger = true;
		colliderDesc.iFilterMask =
		{
			PHYSICSFILTERGROUP::Enum::PLAYER
			| PHYSICSFILTERGROUP::Enum::MAP
		};
		colliderDesc.bIsTrigger = true;
		colliderDesc.bSetOnlyFilter = false;
		colliderDesc.bIsActive = true;
		colliderDesc.vCenter = { 0.f, 1.f, 0.f };
		colliderDesc.vExtents = { 10.f, 3.f, 4.f };
		colliderDesc.strAttackPresetTag = "Lianhuo_XSpace";
		PHYSICSMATERIAL_DESC mtrlDesc{};
		mtrlDesc.eMaterial = EPhysicsMaterial::CONCRETE;
		colliderDesc.tMaterial = mtrlDesc;

		PHYSICSRIGIDBODY_DESC rigidbodyDesc{};
		rigidbodyDesc.eType = EPhysicsActorType::KINEMATIC;
		rigidbodyDesc.detection = EPhysicsCollisionDetection::DISCRETE;
		rigidbodyDesc.bUseGravity = false;
		rigidbodyDesc.bIsKinematic = true;

		// STRIKE
		{
			if (FAILED(Add_CollideModule(
				ENUM_TO_UINT(EState::STRIKE),
				&colliderDesc,
				&rigidbodyDesc)))
				return E_FAIL;
		}
	}
	return S_OK;
}

CLianhuo_XSpace* CLianhuo_XSpace::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CLianhuo_XSpace* pInstance = new CLianhuo_XSpace(pDevice, pDeviceContext);
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("CLianhuo_XSpace::Create, Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CLianhuo_XSpace::Clone(void* pArg)
{
	CLianhuo_XSpace* pInstance = new CLianhuo_XSpace(*this);
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("CLianhuo_XSpace::Clone, Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CLianhuo_XSpace::Free()
{
	Super::Free();
}