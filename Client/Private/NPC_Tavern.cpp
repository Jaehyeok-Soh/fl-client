...
CNPC_Tavern::CNPC_Tavern(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: Super(pDevice, pDeviceContext)
{
}

CNPC_Tavern::CNPC_Tavern(const CNPC_Tavern& rhs)
	: Super(rhs)
{
}

HRESULT CNPC_Tavern::Initialize_Prototype()
{
	if (FAILED(Super::Initialize_Prototype()))
		return E_FAIL;

	Set_Object_Enum_Tag(OBJECT_ENUM_TAG::NPC_TAVERN);

	return S_OK;
}

HRESULT CNPC_Tavern::Initialize(void* pArg)
{
...
HRESULT CNPC_Tavern::Awake(const _uint iCurrentLevelID)
{
...
void CNPC_Tavern::Update_Priority(const _float fTimeDelta)
{
	Super::Update_Priority(fTimeDelta);
}

void CNPC_Tavern::Update(const _float fTimeDelta)
{
	Super::Update(fTimeDelta);
}

void CNPC_Tavern::Update_Late(const _float fTimeDelta)
{
	Super::Update_Late(fTimeDelta);
}

void CNPC_Tavern::Ready_Before_Render(const _float fTimeDelta)
{
	Super::Ready_Before_Render(fTimeDelta);
}

HRESULT CNPC_Tavern::Render()
{
...
void CNPC_Tavern::OnCollision(_uint iMyColliderLayer, _uint iOtherLayer, CGameObject* pOther)
{
	Super::OnCollision(iMyColliderLayer, iOtherLayer, pOther);
}

void CNPC_Tavern::OnCollision_Enter(_uint iMyColliderLayer, _uint iOtherLayer, CGameObject* pOther, const COL_HIT_INFO& tHitInfo)
{
	Super::OnCollision_Enter(iMyColliderLayer, iOtherLayer, pOther, tHitInfo);
}

void CNPC_Tavern::OnCollision_Exit(_uint iMyColliderLayer, _uint iOtherLayer, CGameObject* pOther)
{
	Super::OnCollision_Exit(iMyColliderLayer, iOtherLayer, pOther);
}

void CNPC_Tavern::OnTrigger_Enter(_uint iMyColliderLayer, _uint iOtherLayer, CGameObject* pOther, const COL_HIT_INFO& tHitInfo)
{
	Super::OnTrigger_Enter(iMyColliderLayer, iOtherLayer, pOther, tHitInfo);
}

void CNPC_Tavern::OnTrigger_Exit(_uint iMyColliderLayer, _uint iOtherLayer, CGameObject* pOther)
{
	Super::OnTrigger_Exit(iMyColliderLayer, iOtherLayer, pOther);
}

_bool CNPC_Tavern::On_Hit(const HIT_DESC& hitDesc)
{
	return true;
}

void CNPC_Tavern::Try_Attack(const HIT_DESC& hitDesc)
{
}

HRESULT CNPC_Tavern::Ready_PartObjects()
{
	return S_OK;
}

HRESULT CNPC_Tavern::Ready_Components(void* pArg)
{
...
CNPC_Base::NPC_DESC CNPC_Tavern::Get_PreSetDesc(_uint iLevelId)
{
...
void CNPC_Tavern::QuestEnter()
{
	Super::QuestEnter();
}

void CNPC_Tavern::QuestExit()
{
	Super::QuestExit();
}

void CNPC_Tavern::Interact()
{
	Super::Interact();
}

CNPC_Tavern* CNPC_Tavern::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CNPC_Tavern* pInsatnce = new CNPC_Tavern(pDevice, pDeviceContext);
	if (FAILED(pInsatnce->Initialize_Prototype()))
	{
		MSG_BOX("CNPC_Tavern::Create, Failed");
...
CGameObject* CNPC_Tavern::Clone(void* pArg)
{
	CNPC_Tavern* pClone = new CNPC_Tavern(*this);
	if (FAILED(pClone->Initialize(pArg)))
	{
		MSG_BOX("CNPC_Tavern::Clone, Failed");
		Safe_Release(pClone);
	}
	return pClone;
}

void CNPC_Tavern::Free()
{
	Super::Free();
}
