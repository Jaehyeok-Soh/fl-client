...
CMonster_Dog_Body::CMonster_Dog_Body(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: Super(pDevice, pDeviceContext)
{
}

CMonster_Dog_Body::CMonster_Dog_Body(const CMonster_Dog_Body& rhs)
	: Super(rhs)
{
}

HRESULT CMonster_Dog_Body::Initialize_Prototype()
{
...
HRESULT CMonster_Dog_Body::Initialize(void* pArg)
{
...
HRESULT CMonster_Dog_Body::Awake(const _uint iCurrentLevelIndex)
{
...
void CMonster_Dog_Body::Update_Priority(_float fTimeDelta)
{
...
void CMonster_Dog_Body::Ready_Before_Render(_float fTimeDelta)
{
	Super::Ready_Before_Render(fTimeDelta);
}

void CMonster_Dog_Body::OnCollision(_uint iMyColliderLayer, _uint iOtherLayer, CGameObject* pOther)
{
	Super::OnCollision(iMyColliderLayer, iOtherLayer, pOther);
}

void CMonster_Dog_Body::OnCollision_Enter(_uint iMyColliderLayer, _uint iOtherLayer, CGameObject* pOther, const COL_HIT_INFO& tHitInfo)
{
	Super::OnCollision_Enter(iMyColliderLayer, iOtherLayer, pOther, tHitInfo);
}

void CMonster_Dog_Body::OnCollision_Exit(_uint iMyColliderLayer, _uint iOtherLayer, CGameObject* pOther)
{
...
_bool CMonster_Dog_Body::On_Hit(const HIT_DESC& hitDesc)
{
	return Super::On_Hit(hitDesc);
}

HRESULT CMonster_Dog_Body::Render()
{
...
HRESULT CMonster_Dog_Body::Ready_Components(MONSTERBODY_DESC* pDesc)
{
	return S_OK;
}

HRESULT CMonster_Dog_Body::Bind_ShaderResources()
{
	return S_OK;
}

HRESULT CMonster_Dog_Body::Ready_DissolveEffect_Setting()
{
...
void CMonster_Dog_Body::DissolveStart()
{
	using DS = DissolveEffectDesc;
	m_tDissolveDesc.Reset();
...
CMonster_Dog_Body* CMonster_Dog_Body::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CMonster_Dog_Body* pInstance = new CMonster_Dog_Body(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("CMonster_Dog_Body::Create, Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CMonster_Dog_Body::Clone(void* pArg)
{
	CMonster_Dog_Body* pInstance = new CMonster_Dog_Body(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("CMonster_Dog_Body::Clone, Failed");
		Safe_Release(pInstance);
	}
