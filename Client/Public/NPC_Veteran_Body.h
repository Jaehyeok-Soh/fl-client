...
class CNPC_Veteran_Body final : public CNPC_Body_Base
{
	using Super = CNPC_Body_Base;
private:
	CNPC_Veteran_Body(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	CNPC_Veteran_Body(const CNPC_Veteran_Body& rhs);
	virtual ~CNPC_Veteran_Body() = default;
...
public:
	static CNPC_Veteran_Body* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
