...
class CCanvas;
class CUITrigger;
class CBuilder_UIPrefabs final : public CBuilderBase
{
	using Super = CBuilderBase;
private:
	CBuilder_UIPrefabs(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, _uint iLevelID);
	virtual ~CBuilder_UIPrefabs() = default;
