...
public:
	...
	HRESULT Swap_MapUILayerCache(uint32_t iLevelIndex, unordered_map<_string, CUILayer*>&& Cache) { if(iLevelIndex >= g_iLevelType_Count)return E_FAIL;m_mapUILayerCache[iLevelIndex].swap(Cache);return S_OK;}
	...
	HRESULT Add_VecUILayerCache(uint32_t iLevelIndex, CUILayer* pCache) { if (iLevelIndex >= g_iLevelType_Count)return E_FAIL; m_vecUILayerCache[iLevelIndex].push_back(pCache); return S_OK; }
	...
	CUILayer* Find_UILayer(uint32_t iLevelIndex, const _string& strUILayerTag);
	CGenericUI* Find_GenericUI(uint32_t iLevelIndex, const _string& strUITag);

	/* 특정 UI 오브젝트가 보관하고 있는 UI 오브젝트들을 가져올 때 */
	vector<CUILayer*>* Find_UILayer_Vector(uint32_t iLevelIndex, const _string& strCanvasTag);
	...
	vector<CUILayer*>* Get_Level_All_UILayer(uint32_t iLevelIndex);
...
private:
	...
	array<unordered_map<_string, CUILayer*>,	g_iLevelType_Count >m_mapUILayerCache;
	...
	array<vector<CUILayer*>, g_iLevelType_Count> m_vecUILayerCache;
