...
class CNPC_Tavern final : public CNPC_Base
{
	using Super = CNPC_Base;

private:
	CNPC_Tavern(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CNPC_Tavern(const CNPC_Tavern& rhs);
	virtual ~CNPC_Tavern() = default;
...
public:
	static CNPC_Tavern* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
