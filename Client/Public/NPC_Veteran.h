...
class CNPC_Veteran final : public CNPC_Base
{
	using Super = CNPC_Base;

private:
	CNPC_Veteran(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CNPC_Veteran(const CNPC_Veteran& rhs);
	virtual ~CNPC_Veteran() = default;
...
public:
	static CNPC_Veteran* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
